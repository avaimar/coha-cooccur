...
for decade, model in tqdm(vectors.items()):
        ...
        for quartile in range(4):
            ...
            if asian_mean_vec is not None and white_mean_vec is not None:
                bias_score, _, _ = bias_utils.compute_bias_score(
                    attribute_vecs=otherization_vecs, t1_mean=white_mean_vec, t2_mean=asian_mean_vec, cosine=cosine)
    ...
    for wl in ['{}_San_Bruno_All', 'PNAS {} Target Words']:
        ...
        for other_w in otherization_df['w'].unique():
            ...
            for group, group_df in group_dict.items():
                ...
                for surname in surnames:
                    # Surnames
                    surname_vec = bias_utils.compute_mean_vector(model, [surname])
                    ...
                    _, t1_component, _ = bias_utils.compute_bias_score(
                        attribute_vecs=otherization_vecs, t1_mean=surname_vec, t2_mean=surname_vec, cosine=cosine)
                    ...
                    quart_dict = {
                        ...
                        '%Deviation Target (quartile)': [surname_dict['relative_quartile'].iloc[0]],
                        '%Deviation Target': [surname_dict['%_deviation_optimum'].iloc[0]],
        ...
        asian_mean_vec = bias_utils.compute_mean_vector(model, asian_surnames)
        white_mean_vec = bias_utils.compute_mean_vector(model, white_surnames)
...
main_df.to_csv(os.path.join(output_path, 'quartile_df_MG{}_CS{}.csv'.format(match_garg, cosine)), index=False)

regression_df['wl'] = regression_df['wl'].apply(lambda wl: wl.replace('_', ' '))
regression_df.to_csv(os.path.join(output_path, 'regression_df_MG{}_CS{}.csv'.format(match_garg, cosine)), index=False)
